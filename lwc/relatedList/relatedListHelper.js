...
export default class RelatedListHelper {
    ...
    processData(data, state){
        ...
        this.generateLinks(records,state.invokedFromExperience)
    ...
    generateLinks(records,invokedFromExperience) {
        records.forEach(record => {
            if(invokedFromExperience){
                let urlString = window.location.href;
                let baseURL = urlString.substring(0, urlString.indexOf("/s"));
                record.LinkName = baseURL+'/s/detail/'+record.Id
            }
            else{
